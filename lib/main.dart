...
import 'screens/fitness_home_screen.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  WeeklyNutritionGoals _weeklyNutritionGoals = WeeklyNutritionGoals();
  late List<DailyEnergyEntry> _dailyEnergyEntries;
  String _userName = 'Friend';
  double? _userHeightCm;
  String? _userAvatarPath;
  WeeklyPlanExtras _weeklyPlanExtras = WeeklyPlanExtras();
  ...
  void _deletePlannedWeeklyPlanInRange(DateTime start, DateTime end) {
    final s = DateTime(start.year, start.month, start.day);
    final e = DateTime(end.year, end.month, end.day);

    setState(() {
      _sessions.removeWhere((ses) {
        if (ses.status != WorkoutStatus.planned) return false;
        if (ses.source != 'weekly_plan') return false;
        final d = DateTime(ses.date.year, ses.date.month, ses.date.day);
  ...
  void _onUpdateBurnedForDate(DateTime date, int burnedKcal) {
    final d = _dateOnly(date);

    setState(() {
      final idx = _dailyEnergyEntries.indexWhere((e) => _dateOnly(e.date) == d);
      if (idx == -1) {
        _dailyEnergyEntries.add(DailyEnergyEntry(date: d, burnedKcal: burnedKcal));
      } else {
        _dailyEnergyEntries[idx] = DailyEnergyEntry(date: d, burnedKcal: burnedKcal);
      }
  ...
  void _loadFromHive() {
    try {
      ...
      final weeklyPlanExtrasMap = widget.box.get('weeklyPlanExtras') as Map?;

      _weeklyPlanExtras = weeklyPlanExtrasMap == null
          ? WeeklyPlanExtras()
          : WeeklyPlanExtras.fromMap(weeklyPlanExtrasMap);

      final weeklyGoalMap =
          widget.box.get('weeklyNutritionGoals') as Map?;

      final weeklyGoalsMap = widget.box.get('weeklyNutritionGoals') as Map?;
      _weeklyNutritionGoals = weeklyGoalsMap == null
          ? WeeklyNutritionGoals()
          : WeeklyNutritionGoals.fromMap(weeklyGoalsMap);
      ...
      // 🔹 restore weekly nutrition goals
      _weeklyNutritionGoals = weeklyGoalMap == null
          ? WeeklyNutritionGoals()
          : WeeklyNutritionGoals.fromMap(weeklyGoalMap);
      ...
      _weeklyNutritionGoals = WeeklyNutritionGoals();
  ...
  Future<void> _saveToHive() async {
    ...
    await widget.box.put('weeklyPlanExtras', _weeklyPlanExtras.toMap());
  }

  void _onUpdateWeeklyPlanExtras(WeeklyPlanExtras extras) {
    setState(() {
      _weeklyPlanExtras = extras;
    });
    _saveToHive();
  }
  ...
  void _onUpdateWeeklyGoals(WeeklyNutritionGoals goals) {
    setState(() {
      _weeklyNutritionGoals = goals;
    });
    _saveToHive();
  }
  ...
  Widget build(BuildContext context) {
    final pages = [
      ...
      FitnessHomeScreen(
        ...
        cookbookMeals: _cookbookMeals,
        weeklyGoals: _weeklyNutritionGoals,
        onUpdateWeeklyGoals: _onUpdateWeeklyGoals,
        weeklyPlanExtras: _weeklyPlanExtras,
        onUpdateWeeklyPlanExtras: _onUpdateWeeklyPlanExtras,
      ),
      ...
      NutritionScreen(
        ...
        burnedForDate: _burnedForDate,
        onUpdateBurnedForDate: _onUpdateBurnedForDate,
        cookbookMeals: _cookbookMeals,
        onAddCookbookMeal: _onAddCookbookMeal,
        weeklyGoals: _weeklyNutritionGoals,                   // ✅ NEW
        onUpdateWeeklyGoals: (newWeekly) {                    // ✅ NEW
          setState(() {
            _weeklyNutritionGoals = newWeekly;
          });
          _saveToHive();
        },
      ),
