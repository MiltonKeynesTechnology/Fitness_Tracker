...
class WorkoutSession {
  ...
  /// NEW: where this session came from:
  /// - 'weekly_plan' (generated from the weekly template)
  /// - 'manual' (created/scheduled by the user outside the plan)
  final String source;
  ...
  factory WorkoutSession.fromMap(Map map) {
    ...
    if (rawStatus == null) {
      ...
      final idx = rawStatus.clamp(0, WorkoutStatus.values.length - 1);
      status = WorkoutStatus.values[idx];
    }

    // ✅ NEW: old sessions won't have this, so default to 'manual'
    final src = (map['source'] as String?) ?? 'manual';
...
String runTargetTypeLabel(RunTargetType t) {
  switch (t) {
...
class RunPlan {
  final String? title; // e.g. "Easy run"
  final RunTargetType targetType;

  final int? minutes;        // time target
  final double? distanceKm;  // distance target
  final String? paceMinPerKm; // e.g. "5:30"
  final int? targetBpm;      // HR target
  final String? notes;
  ...
  factory RunPlan.fromMap(Map map) {
    final idx = (map['targetType'] as int?) ?? RunTargetType.time.index;
    final safeIdx = idx.clamp(0, RunTargetType.values.length - 1);
    return RunPlan(
      title: map['title'] as String?,
      targetType: RunTargetType.values[safeIdx],
      minutes: map['minutes'] as int?,
      distanceKm: (map['distanceKm'] as num?)?.toDouble(),
      paceMinPerKm: map['paceMinPerKm'] as String?,
      targetBpm: map['targetBpm'] as int?,
      notes: map['notes'] as String?,
    );
  }
}

class WeeklyPlanDayExtras {
  // These map directly into NutritionScreen via WeeklyNutritionGoals
  // (we store the goal here too for convenience, but we’ll also write it to WeeklyNutritionGoals)
  final NutritionGoal? nutritionGoal;

  // Simple flexible list for now (e.g. ["Fiber", "Iron", "Omega-3"])
  final List<String> microGoals;

  // MealType.name -> list of cookbook meal IDs
  final Map<String, List<String>> mealsByType;

  final RunPlan? runPlan;

  WeeklyPlanDayExtras({
    this.nutritionGoal,
    this.microGoals = const [],
...
static const _noChange = Object();
  ...
  WeeklyPlanDayExtras copyWith({
    Object? nutritionGoal = _noChange, // pass NutritionGoal? OR null to clear
    List<String>? microGoals,
    Map<String, List<String>>? mealsByType,
    Object? runPlan = _noChange,       // pass RunPlan? OR null to clear
  }) {
    return WeeklyPlanDayExtras(
      nutritionGoal: identical(nutritionGoal, _noChange)
          ? this.nutritionGoal
          : nutritionGoal as NutritionGoal?,
      microGoals: microGoals ?? this.microGoals,
      mealsByType: mealsByType ?? this.mealsByType,
      runPlan: identical(runPlan, _noChange) ? this.runPlan : runPlan as RunPlan?,
    );
  }

  Map<String, dynamic> toMap() => {
        'nutritionGoal': nutritionGoal?.toMap(),
        'microGoals': microGoals,
        'mealsByType': mealsByType,
        'runPlan': runPlan?.toMap(),
      };

  factory WeeklyPlanDayExtras.fromMap(Map map) {
    final rawMicros = (map['microGoals'] as List?) ?? [];
    final rawMeals = (map['mealsByType'] as Map?) ?? {};

    final mealsConverted = <String, List<String>>{};
    rawMeals.forEach((key, value) {
      final list = (value as List?)?.map((e) => e.toString()).toList() ?? <String>[];
      mealsConverted[key.toString()] = list;
    });

    return WeeklyPlanDayExtras(
      nutritionGoal: map['nutritionGoal'] == null
          ? null
          : NutritionGoal.fromMap(map['nutritionGoal'] as Map),
      microGoals: rawMicros.map((e) => e.toString()).toList(),
      mealsByType: mealsConverted,
      runPlan: map['runPlan'] == null ? null : RunPlan.fromMap(map['runPlan'] as Map),
    );
  }
}

class WeeklyPlanExtras {
  // Key: weekday int (DateTime.monday..DateTime.sunday)
  final Map<int, WeeklyPlanDayExtras> byWeekday;

  WeeklyPlanExtras({Map<int, WeeklyPlanDayExtras>? byWeekday})
      : byWeekday = byWeekday ??
            {
              DateTime.monday: WeeklyPlanDayExtras(),
              DateTime.tuesday: WeeklyPlanDayExtras(),
              DateTime.wednesday: WeeklyPlanDayExtras(),
              DateTime.thursday: WeeklyPlanDayExtras(),
              DateTime.friday: WeeklyPlanDayExtras(),
              DateTime.saturday: WeeklyPlanDayExtras(),
              DateTime.sunday: WeeklyPlanDayExtras(),
            };

  WeeklyPlanDayExtras forWeekday(int weekday) {
    return byWeekday[weekday] ?? WeeklyPlanDayExtras();
  }

  WeeklyPlanExtras copyWithDay(int weekday, WeeklyPlanDayExtras day) {
    final next = Map<int, WeeklyPlanDayExtras>.from(byWeekday);
    next[weekday] = day;
    return WeeklyPlanExtras(byWeekday: next);
  }

  Map<String, dynamic> toMap() {
    return {
      'byWeekday': byWeekday.map((k, v) => MapEntry(k.toString(), v.toMap())),
    };
  }

  factory WeeklyPlanExtras.fromMap(Map map) {
    final raw = map['byWeekday'] as Map?;
    if (raw == null) return WeeklyPlanExtras();

    final converted = <int, WeeklyPlanDayExtras>{};
    raw.forEach((key, value) {
      final weekday = int.tryParse(key.toString());
      if (weekday == null) return;
      if (value is Map) {
        converted[weekday] = WeeklyPlanDayExtras.fromMap(value);
      }
    });

    return WeeklyPlanExtras(byWeekday: converted);
  }
}
