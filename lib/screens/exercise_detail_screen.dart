...
class _ExerciseDetailScreenState extends State<ExerciseDetailScreen> {
  ...
  Future<void> _showSetPrDialog(BuildContext context) async {
    ...
    final updated = Exercise(
      id: _exercise.id,
      name: _exercise.name,
      region: _exercise.region,
      group: _exercise.group,
      subGroup: _exercise.subGroup,
      goalWeight: _exercise.goalWeight,
      goalReps: _exercise.goalReps,
      manualPr: result,
    );
  ...
  // ---- GOAL SECTION (based on estimated 1RM) ----
    ...
    for (final s in widget.sessions) {
      for (final we in s.exercises) {
        ...
        for (final set in we.sets) {
          if (set.weight == null) continue; // ignore pure bodyweight
          final w = set.weight!;
          final r = set.reps;
...
double _estimate1RM(double weight, int reps) {
  // Simple Epley formula, clamp reps to avoid silly numbers.
...
String? _computePrInfo(Exercise exercise, List<double> weightValues) {
  ...
  if (prevBest == null) {
    // Only one logged point and no manual before it
