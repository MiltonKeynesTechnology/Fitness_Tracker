...
class _LogMealScreenState extends State<LogMealScreen> {
  ...
  // final TextEditingController _cookbookSearchCtrl = TextEditingController();
  // String _cookbookQuery = '';
  // String? _selectedCookbookId; // or CookbookMeal? if you're storing the object
  ...
  void initState() {
    ...
    _cookbookSearchController.addListener(() {
      setState(() {});
    });
  ...
  Widget build(BuildContext context) {
    ...
    final filteredMeals = _filteredCookbookMeals;

    // If the current selection disappears due to filtering, clear it safely
    if (_selectedCookbookMealId != null &&
        !filteredMeals.any((m) => m.id == _selectedCookbookMealId)) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
        setState(() => _selectedCookbookMealId = null);
      });
    }
      ...
      body: ListView(
        ...
        children: [
          ...
          if (widget.cookbookMeals.isNotEmpty) ...[
            Text('From your cookbook', style: theme.textTheme.titleSmall),
            const SizedBox(height: 8),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                suffixIcon: _cookbookSearchController.text.isEmpty
                    ? null
                    : IconButton(
                        icon: const Icon(Icons.clear),
                        onPressed: () => _cookbookSearchController.clear(),
                      ),
            ...
            DropdownButtonFormField<String>(
              ...
              items: filteredMeals.map((meal) {
                return DropdownMenuItem<String>(
                  value: meal.id,
                  child: Text(meal.title, overflow: TextOverflow.ellipsis),
                );
              }).toList(),
              onChanged: (value) {
                if (value == null) return;
                setState(() => _selectedCookbookMealId = value);

                final match = filteredMeals.firstWhere((m) => m.id == value);
                _applyCookbookMeal(match);
              },
            ),
