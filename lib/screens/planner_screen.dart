...
class _PlannerScreenState extends State<PlannerScreen> {
  ...
  late WeeklyNutritionGoals _weeklyGoalsDraft;
  late WeeklyPlanExtras _weeklyPlanExtrasDraft;
  ...
  void initState() {
    ...
    _weeklyGoalsDraft = widget.weeklyGoals;
    _weeklyPlanExtrasDraft = widget.weeklyPlanExtras;
    ...
    // Prefill from existing weekly-plan sessions (next 7 days from today),
    // so the editor reflects what the user currently sees as their plan.
  ...
  String _workoutSummaryForWeekday(int weekday) {
    final custom = _customPlans[weekday];
    if (custom != null) {
      return '${custom.name} • ${custom.exercises.length} exercises';
    }

    final templates = _templateOptions;
    final selectedTemplate = _templateById(_templateIds[weekday], templates);
    if (selectedTemplate != null) {
      return '${selectedTemplate.name} • ${selectedTemplate.exercises.length} exercises';
    }

    return 'Rest day / no exercises';
  }

  Future<void> _editDayDetails(int weekday) async {
    final result = await Navigator.of(context).push<DayPlanEditResult>(
      MaterialPageRoute(
        builder: (_) => DayPlanEditorScreen(
          weekday: weekday,
          weeklyGoals: _weeklyGoalsDraft,
          weeklyPlanExtras: _weeklyPlanExtrasDraft,
          cookbookMeals: widget.cookbookMeals,

          // Workout hook (uses YOUR existing workout editor flow)
          workoutSummary: _workoutSummaryForWeekday(weekday),
          onEditWorkout: () async {
            // This opens your current "Custom" workout bottom-sheet editor
            await _editCustomPlanFor(weekday);

            // NOTE: the summary inside DayPlanEditorScreen won't live-update
            // because it's passed as a String. It will be correct next time you open it.
            // That's OK for now.
          },
        ),
      ),
    ...
    setState(() {
      _weeklyGoalsDraft = result.weeklyGoals;
      _weeklyPlanExtrasDraft = result.weeklyPlanExtras;
    });
  }
  ...
  WorkoutSession? _templateById(String? id, List<WorkoutSession> templates) {
                    ...
                    children: [
                      ...
                      TextField(
                        ...
                        decoration: const InputDecoration(
                          ...
                          hintText: 'e.g. Upper 1, Lower heavy, Arms',
                        ),
                      ),
                            ...
                            onPressed: () {
                              ...
                              if (chosen.isEmpty) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text(
                                      'Pick at least one exercise for this day.',
                                    ),
                                  ),
                                );
                                return;
  ...
  DateTime _effectiveEndDate() {
    ...
    switch (_mode) {
      ...
      case _PlanMode.indefinite:
        // Must generate a finite horizon (sessions stored as concrete dates).
        return start.add(const Duration(days: (12 * 7) - 1));
    }
  }

  void _copyFromPreviousDay(int weekday) {
    if (weekday == DateTime.monday) return;
    final prev = weekday - 1;

    setState(() {
      _customPlans[weekday] = _customPlans[prev];
      _templateIds[weekday] = _templateIds[prev];
    });
  }
  ...
  void _applyWeeklyPlan() {
    ...
    // 1) Remove existing weekly-plan sessions in range (REPLACE semantics).
    ...
    for (int offset = 0; offset < totalDays; offset++) {
      ...
      if (templateId == null && custom == null) continue; // rest
    ...
    widget.onUpdateWeeklyGoals(_weeklyGoalsDraft);
    widget.onUpdateWeeklyPlanExtras(_weeklyPlanExtrasDraft);

    Navigator.of(context).pop({'removed': toRemove.length, 'added': newSessions.length});
  }
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            Text(
              'Weekly plan editor (Mon–Sun). This sets ONLY your weekly plan workouts.',
              style: theme.textTheme.bodySmall,
            ),
            const SizedBox(height: 12),
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  if (custom != null) {
                    ...
                    sublabel = 'Template: ${selectedTemplate.name} • ${selectedTemplate.exercises.length} exercises';
                  } else {
                    sublabel = 'Rest day';
                  }
                        ...
                        child: Column(
                          ...
                          children: [
                                  ...
                                  child: DropdownButtonFormField<WorkoutSession?>(
                                    ...
                                    onChanged: (v) {
                                      setState(() {
                                        ...
                                        if (v != null) _customPlans[weekday] = null;
                                      });
                                    },
                                  ),
                                ),
                            ...
                            Row(
                              children: [
                                OutlinedButton.icon(
                                  ...
                                  label: Text(custom == null ? 'Custom' : 'Edit'),
                                ),
                                const SizedBox(width: 8),
                                OutlinedButton.icon(
                                  onPressed: () => _editDayDetails(weekday),
                                  icon: const Icon(Icons.edit_calendar),
                                  label: const Text('Day details'),
                                ),
                                const SizedBox(width: 8),
                                OutlinedButton.icon(
                                  onPressed: weekday == DateTime.monday
                                      ? null
                                      : () => _copyFromPreviousDay(weekday),
                            ...
                            if (custom != null) ...[
                              ...
                              if (custom.exercises.length > 10)
                                Padding(
                                  padding: const EdgeInsets.only(top: 6),
                                  child: Text(
                                    '+ ${custom.exercises.length - 10} more…',
                                    style: theme.textTheme.bodySmall,
                                  ),
                                ),
                            ],
